...
io.on('connection', socket=>{
  ...
  socket.on('joinRoom', ({roomId, name}, cb)=>{
    ...
    if (!room) return cb && cb({ok:false, err:'Комната не найдена'});
    if (room.players[socket.id]) return cb && cb({ok:false, err:'Уже в комнате'});
    const player = {id: socket.id, name: name || 'Игрок'};
  ...
  socket.on('startGame', ({roomId, settings}, cb)=>{
    ...
    if (socket.id !== room.host) return cb && cb({ok:false, err:'Только хозяин может начать игру'});
    const playerCount = Object.keys(room.players).length;
    if (playerCount < 2) return cb && cb({ok:false, err:'Нужно минимум 2 игрока'});
    if (settings) {
  ...
  socket.on('submitPrompt', ({roomId, prompt}, cb)=>{
    const room = rooms[roomId];
    if (!room) return cb && cb({ok:false, err:'Комната не найдена'});
    if (room.phase !== 'writing') return cb && cb({ok:false, err:'Не время для подсказок'});
    if (room.history.find(h=>h.owner===socket.id)) return cb && cb({ok:false, err:'Вы уже отправили подсказку'});
    ...
    cb && cb({ok:true});
    if (room.history.length === Object.keys(room.players).length) {
      moveToDrawing(roomId);
    }
  });
  ...
  socket.on('drawingData', ({roomId, targetId, strokes}, cb)=>{
    const room = rooms[roomId];
    if (!room) return cb && cb({ok:false});
    if (room.phase !== 'drawing') return cb && cb({ok:false, err:'Не время рисовать'});
    const expectedDrawer = getDrawerForOwner(room, targetId);
    if (expectedDrawer !== socket.id) return cb && cb({ok:false, err:'Вы не назначены рисовать для этого игрока'});
    const entry = room.history.find(h=>h.owner===targetId);
    if (!entry) return cb && cb({ok:false, err:'Не найдено'});
    ...
    if (allDrawings === Object.keys(room.players).length) {
      moveToGuessing(roomId);
    }
  });
  ...
  socket.on('submitGuess', ({roomId, targetId, guess}, cb)=>{
    const room = rooms[roomId];
    if (!room) return cb && cb({ok:false});
    if (room.phase !== 'guessing') return cb && cb({ok:false, err:'Не время угадывать'});
    const expectedGuesser = getGuesserForOwner(room, targetId);
    if (expectedGuesser !== socket.id) return cb && cb({ok:false, err:'Вы не назначены угадывать для этого рисунка'});
    const entry = room.history.find(h=>h.owner===targetId);
    ...
    if (allGuesses === Object.keys(room.players).length) {
      moveToReveal(roomId);
    }
  });
  ...
  function startPhaseTimer(roomId, phase) {
    ...
    room.timers.interval = setInterval(()=>{
      ...
      io.to(roomId).emit('timerTick', {remaining: room.timers.phase.remaining});
  ...
  function moveToDrawing(roomId) {
    ...
    for (const pid of room.order) {
      if (!room.history.find(h=>h.owner===pid)) {
        room.history.push({owner: pid, sequence:[{type:'prompt', data:'(нет ответа)'}]});
      }
    }
